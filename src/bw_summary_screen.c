...
const u16 sSummaryScreen_Pal_BW_AWR[]                       = INCBIN_U16("graphics/summary_screen/bw/tiles_awr.gbapal");
...
void ShowPokemonSummaryScreen_BW(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ...
    if (mode == SUMMARY_MODE_BOX
     || mode == SUMMARY_MODE_BOX_SELECT_MOVE
     || mode == SUMMARY_MODE_BOX_RELEARNER_BATTLE
     || mode == SUMMARY_MODE_BOX_RELEARNER_CONTEST)
...
void ShowSelectMoveBoxPokemonSummaryScreen_BW(struct BoxPokemon *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void), u16 newMove)
{
    ShowPokemonSummaryScreen_BW(SUMMARY_MODE_BOX_SELECT_MOVE, mons, monIndex, maxMonIndex, callback);
    sMonSummaryScreen->newMove = newMove;
}
...
static bool8 LoadGraphics(void)
{
    ...
    case 13:
        ...
        else if (sMonSummaryScreen->mode == SUMMARY_MODE_RELEARNER_BATTLE || sMonSummaryScreen->mode == SUMMARY_MODE_BOX_RELEARNER_BATTLE) // load the appropriate moves page when returning from move relearner
    ...
    case 15:
        ResetSpriteIds();
        CreateMoveTypeIcons();
        if ((sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE || sMonSummaryScreen->mode == SUMMARY_MODE_BOX_SELECT_MOVE) && BW_SUMMARY_CATEGORY_ICONS)
...
static bool8 DecompressGraphics(void)
{
    ...
    case 8:
        if (IS_AWR)
            LoadPalette(sSummaryScreen_Pal_BW_AWR, BG_PLTT_ID(0), 8 * PLTT_SIZE_4BPP);
        else
...
static void SetContestMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (summary->moves[i] != MOVE_NONE)
            SetTypeSpritePosAndPal(NUMBER_OF_MON_TYPES + gMovesInfo[summary->moves[i]].contestCategory, 8, 18 + (i * 28), i + SPRITE_ARR_ID_TYPE);
        else
            SetSpriteInvisibility(i + SPRITE_ARR_ID_TYPE, TRUE);
    }
}
