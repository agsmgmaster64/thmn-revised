...
{
    if ((GetBattlerAbility(cv->battlerAtk) == ABILITY_TRUANT || GetBattlerAbility(cv->battlerAtk) == ABILITY_LAZY) && gBattleMons[cv->battlerAtk].volatiles.truantCounter)
    {
        CancelMultiTurnMoves(cv->battlerAtk);
...
static enum CancelerResult CancelerMoveFailure(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_INVOCATION:
        if (gBattleMons[cv->battlerAtk].volatiles.cursed)
            battleScript = BattleScript_ButItFailed;
        break;
    case EFFECT_PROTECT:
    case EFFECT_ENDURE:
...
static enum MoveEndResult MoveEndMoveBlock(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_KNOCK_OFF:
         ...
         && !NoAliveMonsForEitherParty())
        {
            ...
            if (cv->abilities[cv->battlerDef] == ABILITY_STICKY_HOLD || cv->abilities[cv->battlerDef] == ABILITY_STRONG_GRIP)
            {
                gBattlerAbility = cv->battlerDef;
    ...
    case EFFECT_STEAL_ITEM:
        ...
        else if (cv->abilities[cv->battlerDef] == ABILITY_STICKY_HOLD || cv->abilities[cv->battlerDef] == ABILITY_STRONG_GRIP)
        {
            BattleScriptCall(BattleScript_StickyHoldActivatesRet);
            gBattlerAbility = cv->battlerDef;
    ...
    case EFFECT_HIT_SWITCH_TARGET:
         ...
         && gBattleStruct->battlerState[cv->battlerDef].commanderSpecies == SPECIES_NONE)
        {
            ...
            if (cv->abilities[cv->battlerDef] == ABILITY_SUCTION_CUPS || cv->abilities[cv->battlerDef] == ABILITY_GATE_KEEPER)
            {
...
static enum MoveEndResult MoveEndEchoAbilities(struct BattleCalcValues *cv)
{
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        enum BattlerId battler = gBattleStruct->eventState.moveEndBattler++;

        if (AbilityBattleEffects(ABILITYEFFECT_ECHO_ABILITIES, battler, cv->abilities[battler], 0, TRUE))
            return MOVEEND_RESULT_RUN_SCRIPT;
    ...
    if (itemToSteal != ITEM_NONE) // valid item to steal
    {
        ...
        for (u32 i = 0; i < gBattlersCount; i++)
        {
              ...
              && CanStealItem(battlerDef, cv->battlerAtk, itemToSteal))
            {
                ...
                if (abilityAtk != ABILITY_STICKY_HOLD || abilityAtk != ABILITY_STRONG_GRIP)
                {
                    if (originalAttackerOnField)
                    {
...
static void TryMoodSwingStatChange(struct BattleCalcValues *cv)
{
    u32 bits = 0;
...
static enum MoveResult StatChangeCanAnyChange(struct BattleCalcValues *cv)
{
    ...
    if (GetMoveEffect(cv->move) == EFFECT_MOOD_SWING)
    {
