...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_LUNACY:
            if (shouldAbilityTrigger)
            {
                i = RandomUniformExcept(RNG_MOODY_INCREASE, STAT_ATK, NUM_BATTLE_STATS - 1, MoodyCantRaiseStat);

                if (CompareStat(battler, i, MAX_STAT_STAGE, CMP_LESS_THAN, gLastUsedAbility))
                {
                    gEffectBattler = gBattlerAbility = battler;
                    SetStatChange(battler, i, 1);
                    BattleScriptCall(BattleScript_AbilityStatChange);
                    effect++;
                }
            }
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_SPEED_BOOST:
                ...
                break;
            case ABILITY_POWER_LEAK:
                if (CompareStat(battler, STAT_SPATK, MIN_STAT_STAGE, CMP_GREATER_THAN, gLastUsedAbility) && !BattlerJustSwitchedIn(battler))
                {
                    gEffectBattler = gBattlerAbility = battler;
                    SetStatChange(battler, STAT_SPATK, -1);
                    BattleScriptCall(BattleScript_AbilityStatChange);
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_REACTIVE:
             ...
             && IsBattlerTurnDamaged(gBattlerTarget, EXCLUDING_SUBSTITUTES))
            {
                ...
                {
                    gEffectBattler = gBattlerAbility = gBattlerTarget;
                    if (CompareStat(gBattlerTarget, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN, ABILITY_NONE))
                        SetStatChange(gBattlerTarget, STAT_DEF, 1);

                    if (CompareStat(gBattlerTarget, STAT_SPDEF, MIN_STAT_STAGE, CMP_GREATER_THAN, ABILITY_NONE))
                        SetStatChange(gBattlerTarget, STAT_SPDEF, -1);

                    if (gSpecialStatuses[gBattlerTarget].statStageAmount > 0)
                        BattleScriptCall(BattleScript_AbilityStatChange);
                    else // Not sure if there is an actual ability popup in this case
                        BattleScriptCall(BattleScript_AbilityPopUp);
                    effect++;
                ...
                {
                    gEffectBattler = gBattlerAbility = gBattlerTarget;
                    if (CompareStat(gBattlerTarget, STAT_SPDEF, MAX_STAT_STAGE, CMP_LESS_THAN, ABILITY_NONE))
                        SetStatChange(gBattlerTarget, STAT_SPDEF, 1);

                    if (CompareStat(gBattlerTarget, STAT_DEF, MIN_STAT_STAGE, CMP_GREATER_THAN, ABILITY_NONE))
                        SetStatChange(gBattlerTarget, STAT_DEF, -1);
             ...
             && !gSideTimers[GetBattlerSide(gBattlerAttacker)].mistTimer)
            {
                if (IsBattleMovePhysical(gCurrentMove) && CompareStat(gBattlerAttacker, STAT_ATK, MIN_STAT_STAGE, CMP_GREATER_THAN, gLastUsedAbility))
                {
                    ...
                    else
                    {
                        ...
                        BattleScriptCall(BattleScript_BuzzerActivate);
                        effect++;
                    }
                }
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (u32 i = 0; i < gBattlersCount; i++)
                {
                    ...
                    if (GetBattlerAbility(targetBattler) == ABILITY_STICKY_HOLD
                     || GetBattlerAbility(targetBattler) == ABILITY_STRONG_GRIP)
                    {
                        effect = FALSE;
                        break;
                    }
        ...
        case ABILITY_EELEVATE:
            {
                ...
                if (ability == ABILITY_BEAST_BOOST || ability == ABILITY_OCCULT_BOOST || ability == ABILITY_EELEVATE)
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (IsMistyTerrainAffected(battlerDef, ctx->abilities[battlerDef], ctx->holdEffects[battlerDef], ctx->terrain) && moveType == TYPE_FAITH)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    ...
    case HOLD_EFFECT_LUSTROUS_ORB:
        if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_PALKIA && (moveType == TYPE_WATER || moveType == TYPE_FAITH))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_ADAMANT_ORB:
        if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_DIALGA && (moveType == TYPE_STEEL || moveType == TYPE_FAITH))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    else if (gBattleMons[battlerAtk].ability == ABILITY_ABERRANT)
    {
        if (IsBattleMovePhysical(move))
        {
    ...
    */
    case ABILITY_WINTER_GIFT:
        if (IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerAtk], ctx->weather, B_WEATHER_ICY_ANY) && !IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
    ...
    case ABILITY_UNWAVERING:
        if (gBattleMons[battlerAtk].status1 & STATUS1_ANY && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case ABILITY_PURE_ENIGMA:
        if (gBattleMons[battlerAtk].species == SPECIES_EXTRA_OKINA)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        else
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_YIN_AND_YANG:
        if ((gBattleMons[battlerAtk].attack < gBattleMons[battlerAtk].spAttack) && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.3));
        else if ((gBattleMons[battlerAtk].attack > gBattleMons[battlerAtk].spAttack) && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.3));
    ...
    case ABILITY_PURIFYING_SALT:
        ...
        break;
    case ABILITY_ATHEISM:
        if (moveType == TYPE_FAITH)
        {
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.25));
            if (ctx->updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_ATHEISM);
        }
        break;
    default:
        break;
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    */
    case ABILITY_WINTER_GIFT:
        if (IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerDef], ctx->weather, B_WEATHER_ICY_ANY) && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SAND_CASTLE:
    ...
    case ABILITY_STORM_SHAWL:
        if (IsBattlerWeatherAffected(ctx->holdEffects[ctx->battlerDef], ctx->weather, B_WEATHER_ANY) && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_PURE_ENIGMA:
        if (gBattleMons[battlerDef].species == SPECIES_EXTRA_OKINA)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        else
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_FROSTBITE
        ...
        && (GetConfig(B_BURN_FACADE_DMG) < GEN_6 || moveEffect != EFFECT_FACADE)
        && ctx->abilities[ctx->battlerAtk] != ABILITY_UNWAVERING)
        return UQ_4_12(0.5);
    return UQ_4_12(1.0);
}
...
s32 CalcCritChanceStage(struct DamageContext *ctx)
{
    ...
    if (critChance != CRITICAL_HIT_BLOCKED && (ctx->abilities[ctx->battlerDef] == ABILITY_BATTLE_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_GAP || ctx->abilities[ctx->battlerDef] == ABILITY_GUARD_ARMOR))
    {
...
s32 CalcCritChanceStageGen1(struct DamageContext *ctx)
{
    ...
    if (ctx->abilities[ctx->battlerDef] == ABILITY_BATTLE_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_GAP || ctx->abilities[ctx->battlerDef] == ABILITY_GUARD_ARMOR)
    {
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerDef, ctx->abilities[ctx->battlerDef]);
        critChance = CRITICAL_HIT_BLOCKED;
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    if ((ctx->moveType == TYPE_GHOST) && (defType == TYPE_NORMAL)
        && (ctx->abilities[ctx->battlerAtk] == ABILITY_LOST_DREAM) && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk]);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    if (((ctx->abilities[ctx->battlerDef] == ABILITY_WONDER_GUARD && modifier <= UQ_4_12(1.0) && !isPresentHealing)
        || (ctx->abilities[ctx->battlerDef] == ABILITY_PLAY_GHOST && ctx->battlerDef == GetPartnerBattler(ctx->battlerAtk))
...
bool32 DoBattlersShareType(enum BattlerId battler1, enum BattlerId battler2)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (types1[i] == TYPE_NONE)
            continue;

        for (j = 0; j < 3; j++)
        {
            if (types2[j] == TYPE_NONE)
                continue;
...
u32 GetTotalAccuracy(struct BattleCalcValues *cv, u32 weather)
{
    ...
    if ((attackerWeather & B_WEATHER_SUN) && MoveHas50AccuracyInSun(cv->move))
        moveAcc = 50;
    // Check Wonder Skin.
    if ((cv->abilities[battlerDef] == ABILITY_WONDER_SKIN || cv->abilities[battlerDef] == ABILITY_WONDER_VEIL) && IsBattleMoveStatus(cv->move) && moveAcc > 50)
        moveAcc = 50;
