...
// The following STATIC_ASSERT will prevent developers from compiling the game if the value of the constant on the left does not fit within the number of bits defined in BoxPokemon (currently located in include/pokemon.h).
...
STATIC_ASSERT(NUM_SPECIES < (1 << 11), BoxPokemon_species_TooSmall);
STATIC_ASSERT(NUMBER_OF_MON_TYPES + 1 <= (1 << 5), BoxPokemon_teraType_TooSmall);
STATIC_ASSERT(ITEMS_COUNT < (1 << 10), BoxPokemon_heldItem_TooSmall);
STATIC_ASSERT(MAX_LEVEL <= 100, BoxPokemon_experience_PotentiallTooSmall); // Maximum of ~2 million exp.
STATIC_ASSERT(POKEBALL_COUNT <= (1 << 6), BoxPokemon_pokeball_TooSmall);
STATIC_ASSERT(MOVES_COUNT_ALL < (1 << 11), BoxPokemon_moves_TooSmall);
STATIC_ASSERT(ARRAY_COUNT(sCompressedStatuses) <= (1 << 4), BoxPokemon_compressedStatus_TooSmall);
STATIC_ASSERT(MAX_LEVEL < (1 << 7), BoxPokemon_metLevel_TooSmall);
STATIC_ASSERT(NUM_VERSIONS < (1 << 4), BoxPokemon_metGame_TooSmall);
STATIC_ASSERT(MAX_DYNAMAX_LEVEL < (1 << 4), BoxPokemon_dynamaxLevel_TooSmall);
STATIC_ASSERT(MAX_PER_STAT_IVS < (1 << 5), BoxPokemon_ivs_TooSmall);
...
void DoMonFrontSpriteAnimation(struct Sprite *sprite, u16 species, bool8 noCry, u8 panModeAnimFlag)
{
    ...
    if ((panModeAnimFlag & SKIP_FRONT_ANIM) || (gBattleTypeFlags & BATTLE_TYPE_GHOST) || gSaveBlock2Ptr->optionsMonAnimations == OPTIONS_GENERAL_OFF)
    {
