...
bool32 IsAdditionalEffectBlocked(enum BattlerId battlerAtk, enum Ability abilityAtk, enum BattlerId battlerDef, enum Ability abilityDef, enum Move move)
{
    ...
    if (abilityDef == ABILITY_FLAWLESS && !IsMoldBreakerTypeAbility(battlerAtk, abilityAtk, move))
        return TRUE;
...
bool32 HasBattlerSideMove(enum BattlerId battlerId, enum Move move)
{
    if (HasMove(battlerId, move))
        return TRUE;
    if (HasPartnerIgnoreFlags(battlerId) && HasMove(GetPartnerBattler(battlerId), move))
        return TRUE;
    return FALSE;
}
...
{
    enum Move predictedMove = GetPredictedMove(battlerAtk, battlerDef, gAiLogicData);
    if (((!IsMoldBreakerTypeAbility(battlerAtk, gAiLogicData->abilities[battlerAtk], move) && (defAbility == ABILITY_SHIELD_DUST || defAbility == ABILITY_INNER_FOCUS || defAbility == ABILITY_FLAWLESS))
    ...
    case ABILITY_FASCINATE:
    {
        ...
        else
        {
            if (HasTwoOpponents(battler))
            {
                ...
                {
                    enum AIScore score1 = IncreaseStatDownScore(battler, GetBattlerLeftFoe(battler), STAT_SPATK);
                    enum AIScore score2 = IncreaseStatDownScore(battler, GetBattlerRightFoe(battler), STAT_SPATK);
                    if (score1 > score2)
                        return score1;
                    else
                        return score2;
            ...
            return IncreaseStatDownScore(battler, GetBattlerLeftFoe(battler), STAT_SPATK);
        }
    }
