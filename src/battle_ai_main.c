...
// the following checks apply to any target (including user)
    ...
    case EFFECT_HELPING_HAND:
        if (!hasPartner
          ...
          || aiData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_INNATE_DREAM
          || aiData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_GAP_BLOCK
          || (aiData->partnerMove != MOVE_NONE && IsBattleMoveStatus(aiData->partnerMove))
...
static s32 AI_HPAware(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         ...
         || (moveType == TYPE_FIRE && gAiLogicData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_FLAME_ABSORB)
         || (moveType == TYPE_GRASS && gAiLogicData->abilities[GetPartnerBattler(battlerAtk)] == ABILITY_FLORA_ABSORB))
        {
            if (gBattleMons[battlerDef].volatiles.healBlockTimer)
                return 0;
